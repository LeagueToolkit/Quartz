        ...
        try {
            ...
            for (const entry of entries) {
                ...
                if (!this.scannedTree[entryHash]) {
                    ...
                    // Unhash entry name using hashtables
                    if (this.hashtables) {
                        this.entryName[entryHash] = BINHasher.hexToRaw(this.hashtables, entryHash) || `Entry_${entryHash}`;
                    } else {
                        this.entryName[entryHash] = `Entry_${entryHash}`;
                    }
                    
                    // Get entry type name from hashtables
                    // entry.type is already a hex string from BINHasher.hashToHex()
                    if (this.hashtables && entry.type && entry.type !== '00000000') {
                        ...
                        // Entry type 0 is valid but typically means "no type" or "default type"
                        // Try to look it up in hashtables, but if not found, use a more descriptive name
                        if (this.hashtables && this.hashtables['hashes.bintypes.txt']?.['00000000']) {
                            ...
                            this.entryTypeName[entryHash] = '0x00000000'; // Keep hex format for clarity
                        }
                    } else {
                        this.entryTypeName[entryHash] = entry.type;
                    }
                }
