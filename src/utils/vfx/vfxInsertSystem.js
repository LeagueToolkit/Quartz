...
function insertVFXSystemIntoFile(originalPy, systemFullContent, desiredSystemName) {
  ...
  // Insert system at the top of the VFX systems section
  // (before the first existing VfxSystemDefinitionData) so new/ported systems appear first.
  let updated = '';
  ...
  if (vfxSystemMatches.length > 0) {
    // Insert before the first VFX system line.
    ...
    console.log('[vfxInsertSystem] Inserted VFX system before first VfxSystemDefinitionData');
  } else {
...
function insertVFXSystemWithPreservedNames(originalPy, systemFullContent, desiredSystemName, donorPyContent = null, options = {}) {
  ...
  // Insert at the top of VFX systems section (before first existing system) to keep new items first.
  let updated = '';
  ...
  if (firstSystemMatch && typeof firstSystemMatch.index === 'number') {
    ...
    // Fallback: no existing systems found, insert before final top-level closing brace if possible.
