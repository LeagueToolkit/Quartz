...
/**
 * Parse a simple property like propName: type = value
 * @param {string} content - Content to search
 * @param {string} propName - Property name
 * @param {string} type - Property type (u8, i16, etc.)
 * @returns {number|null}
 */
function parseSimpleProperty(content, propName, type) {
    const regex = new RegExp(
        `${propName}:\\s*${type}\\s*=\\s*([\\d.\\-]+)`,
        'i'
    );
