...
export default function PortAllModeModal({ open, onClose, onSelectMode, donorCount = 0 }: PortAllModeModalProps) {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, zIndex: 5200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)' }} />
            <div
            ...
            >
                ...
                <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ color: 'rgba(255,255,255,0.72)', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.78rem', lineHeight: 1.6 }}>
                        Donor systems detected: <span style={{ color: 'var(--accent2)', fontWeight: 700 }}>{donorCount}</span>
                    </div>
                    <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(255,255,255,0.02)', padding: 14, display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ color: 'var(--accent2)', fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase' }}>Choose Mode</div>
                        <button onClick={() => onSelectMode('normal')} style={{ ...btnBase, background: 'color-mix(in srgb, var(--accent2), transparent 90%)', color: 'var(--accent2)' }}>
                            Port All Normally
                        </button>
                        <button onClick={() => onSelectMode('replace-target')} style={{ ...btnBase, background: 'rgba(239,68,68,0.14)', border: '1px solid rgba(239,68,68,0.32)', color: '#ff7f7f' }}>
