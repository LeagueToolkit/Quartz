...
import { BONE_NAMES, extractExistingIdleParticles } from '../../../../utils/vfx/mutations/idleParticlesManager.js';
...
const mkBone = (value = 'head', customValue = '') => ({ id: Date.now() + Math.random(), boneName: value, customBoneName: customValue });

function parseResolverMap(pyContent) {
  const map = new Map();
...
function CustomDropdown({ value, options, onChange, placeholder = 'Select system...' }) {
...
export default function IdleParticlesManagerModal({
  open,
  onClose,
  targetSystems,
  targetPyContent,
  ...
  const [boneList, setBoneList] = useState([mkBone('head')]);

  const existing = useMemo(
    () => (open ? extractExistingIdleParticles(targetPyContent || '') : []),
    [open, targetPyContent]
  );
  ...
  const resolverMap = useMemo(
    () => (open ? parseResolverMap(targetPyContent || '') : new Map()),
    [open, targetPyContent]
  );
  ...
  const filteredExisting = useMemo(() => {
    ...
    return existing.filter((e) =>
      String(effectKeyToSystem.get(e.effectKey)?.label || '').toLowerCase().includes(term) ||
      (e.bones || []).some((b) => String(b).toLowerCase().includes(term))
    );
  ...
  const handleApply = () => {
    ...
    if (!selected) return;
    const configs = boneList
      .map((b) => ({ boneName: (b.customBoneName || '').trim() || b.boneName }))
      .filter((b) => !!b.boneName);
    onUpsertSystem?.(selected.value, selected.label, configs);
  };

  const handleLoadExisting = (entry) => {
      ...
      >
        ...
        <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          <div style={{
            ...
            <div style={{ color: 'var(--accent2)', fontWeight: 700, marginBottom: 10, fontSize: '0.84rem' }}>
              Existing Idle Entries ({existing.length})
            </div>
            <input
              value={search}
            ...
            {filteredExisting.map((e) => (
              ...
              <div key={e.effectKey} style={{
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  ...
                  <button style={btn('#ff6b6b')} onClick={() => onRemoveEffectKey?.(e.effectKey)}>Remove All</button>
                </div>
              </div>
                );
              })()
          ...
          <div style={{ flex: 1, overflow: 'auto', padding: 14 }}>
            <div style={{ color: 'var(--accent)', fontWeight: 700, marginBottom: 10, fontSize: '0.84rem' }}>
              Add / Update Idle Particles
            </div>

            <div style={{ marginBottom: 8, color: 'rgba(255,255,255,0.75)', fontSize: '0.78rem' }}>
              VFX System
            </div>
            <div style={{ marginBottom: 12 }}>
              <CustomDropdown
                value={selectedSystemKey}
                onChange={setSelectedSystemKey}
            ...
            {boneList.map((item) => (
              <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 8, marginBottom: 8 }}>
                <CustomDropdown
                  value={item.boneName}
                  onChange={(value) =>
                    setBoneList((prev) => prev.map((b) => b.id === item.id ? { ...b, boneName: value } : b))
                  }
                ...
                <button
                  style={btn('#ff6b6b')}
                  onClick={() => setBoneList((prev) => prev.filter((b) => b.id !== item.id))}
                >
                  Remove
                </button>
              </div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
              <button style={btn('#22d3ee')} onClick={() => setBoneList((prev) => [...prev, mkBone('head')])}>Add Bone</button>
            </div>
