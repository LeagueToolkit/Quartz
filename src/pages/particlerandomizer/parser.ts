...
import { parseStaticMaterials, hasStaticMaterials, type StaticMaterial } from './staticMaterialParser';
...
export function parseVfxFile(content: string): ParsedFile {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (currentSystem && /VfxEmitterDefinitionData\s*\{/i.test(trimmed)) {
            ...
            currentEmitter = {
                ...
                meshes: [],
                blendMode: 0,
                colors: { color: null, birthColor: null, fresnelColor: null, lingerColor: null },
            };
                ...
                if (vecMatch) {
                    ...
                    if (vals.length >= 4 && vals.every((n) => !isNaN(n))) {
                        currentEmitter.colors.fresnelColor = {
                            type: 'fresnelColor', lineStart: i, constantLine: i, valuesLines: [], timesLines: [], values: [vals], times: [0], isSimpleVec4: true,
                        };
                    }
                }
            }
    ...
    void colorValueLineNums; // parity with original (tracked but unused downstream)
    ...
    console.log(
        `[Paint2 Parser] Parsed ${result.stats.totalLines} lines, ${result.stats.systemCount} systems, ${result.stats.emitterCount} emitters, ${result.stats.materialCount} materials in ${result.stats.parseTimeMs.toFixed(2)}ms`
    );
