...
/**
 * Best-effort memory hint for tuning concurrency.
 * @returns {number}
 */
function getRuntimeMemoryGB() {
    if (typeof navigator !== 'undefined' && typeof navigator.deviceMemory === 'number') {
        return navigator.deviceMemory;
    }
    // Electron often doesn't expose deviceMemory; use a balanced default.
    return 8;
}

/**
 * Adaptive pipeline settings based on chunk count + memory hint.
 * @param {number} chunkCount
 * @returns {{ pipelineBatch: number, writeBatch: number, writeConcurrency: number }}
 */
function getAdaptiveBatchConfig(chunkCount) {
    const memoryGB = getRuntimeMemoryGB();
    const lowMem = memoryGB <= 4;
    const midMem = memoryGB > 4 && memoryGB <= 8;

    let pipelineBatch = lowMem ? 160 : (midMem ? 240 : 300);
    let writeBatch = lowMem ? 110 : (midMem ? 170 : 220);
    let writeConcurrency = lowMem ? 20 : (midMem ? 28 : 32);

    if (chunkCount < 500) {
        pipelineBatch = Math.min(pipelineBatch, 160);
        writeBatch = Math.min(writeBatch, 110);
    } else if (chunkCount > 5000) {
        pipelineBatch = Math.max(pipelineBatch, 300);
        writeBatch = Math.max(writeBatch, 220);
    }

    return {
        pipelineBatch: Math.max(100, Math.min(pipelineBatch, 360)),
        writeBatch: Math.max(80, Math.min(writeBatch, 260)),
        writeConcurrency: Math.max(8, Math.min(writeConcurrency, 32)),
    };
}
...
export async function unpackWAD(wadFilePath, outputDir, hashtables = null, filter = null, progressCallback = null) {
    ...
    const dirsWithBinFiles = new Set();
    ...
    await Promise.all(Array.from(allDirs).map(async (dirPath) => {
        try {
            ...
            if (isOneDrivePath && dirsWithBinFiles.has(dirPath)) {
    ...
    } = getAdaptiveBatchConfig(chunksToProcess.length);
    console.log(`[WADTool] Adaptive batching: pipeline=${PIPELINE_BATCH}, writeBatch=${WRITE_BATCH}, writeConcurrency=${WRITE_CONCURRENCY}`);
    
    // Helper function to write a batch of files with bounded async concurrency.
    const writeBatch = async (writeQueue) => {
        if (writeQueue.length === 0) return 0;

        const results = await mapWithConcurrency(writeQueue, WRITE_CONCURRENCY, async ({ filePath, chunk }) => {
            try {
    ...
    for (let i = 0; i < chunksToProcess.length; i += PIPELINE_BATCH) {
        ...
        for (const chunk of batch) {
            if (chunk.data) {
                ...
                writeQueue.push({ filePath, chunk });
            }
        }
