...
const ModernSettings = () => {
  ...
  // Windows Integration - Context Menu handlers
  useEffect(() => {
  ...
  const handleToggleContextMenu = async (enabled) => {
    if (!window.require) {
    ...
    try {
      ...
      setContextMenuEnabled(!enabled); // Revert on error
    } finally {
  ...
  const loadCursorFiles = async () => {
    ...
    try {
      ...
      const cursorsDir = path.join(resourcesPath, 'cursors');
      ...
      case 'windowsIntegration':
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {/* Info Section */}
            <FormGroup
              label="About Windows Integration"
              description="How the context menu works"
            >
              <div style={{
                background: 'rgba(255, 255, 255, 0.02)',
                border: '1px solid rgba(255, 255, 255, 0.06)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <FormGroup label="Available Pages">
              <div style={{
                ...
                <div style={{ height: '1px', background: 'rgba(255, 255, 255, 0.06)', margin: '8px 0' }} />
                <ToggleSwitch
                  label="Particle Randomizer"
                  checked={settings.particleRandomizerEnabled}
                  onChange={(checked) => updateSetting('particleRandomizerEnabled', checked)}
                  compact
                />
              </div>
            </FormGroup>
