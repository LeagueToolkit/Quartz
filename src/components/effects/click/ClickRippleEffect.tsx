...
export function ClickRippleEffect({ enabled = true }: { enabled: boolean }) {
    const canvasRef = useRef<HTMLCanvasElement | null>(null);
    const requestRef = useRef<number | null>(null);
    const widthRef = useRef(0);
    const heightRef = useRef(0);
    const buffer1Ref = useRef<Float32Array>(new Float32Array(0));
    const buffer2Ref = useRef<Float32Array>(new Float32Array(0));
    const damping = 0.92;
