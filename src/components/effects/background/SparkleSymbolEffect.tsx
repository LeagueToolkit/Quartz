...
const createWorker = () => new Worker(new URL('./sparkleSymbol.worker.ts', import.meta.url), { type: 'module' });
...
const runMainThreadFallback = (canvas: HTMLCanvasElement) => {
  ...
  const initSymbol = (symbol: Symbol, spawnAbove = false) => {
...
export function SparkleSymbolEffect({ enabled }: { enabled: boolean }) {
