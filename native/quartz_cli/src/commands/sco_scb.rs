...
// dependency on ltk_mesh because the SCO ASCII path doesn't live there yet
// and the flag/central-point semantics differ from StaticMesh.
//
// Game-compat behavior matched from the Python tool:
//   * subtract CentralPoint from every position+pivot, then central = 0,
//     because the old SCO engine implicitly did the subtraction at load
//     while SCB uses vertices as-is.
//   * flags = 5 (Riot's canonical SCB flag value).
...
fn parse_sco(bytes: &[u8]) -> Result<Sco, String> {
    let text = std::str::from_utf8(bytes)
        .map_err(|e| format!("SCO is not valid UTF-8: {e}"))?;
    ...
    while i < lines.len() {
        ...
        match parts[0] {
            ...
            "CentralPoint=" => {
                if parts.len() >= 4 {
                    sco.central = [
                        parts[1].parse().map_err(|e| format!("Bad CentralPoint.x: {e}"))?,
                        parts[2].parse().map_err(|e| format!("Bad CentralPoint.y: {e}"))?,
                        parts[3].parse().map_err(|e| format!("Bad CentralPoint.z: {e}"))?,
                    ];
                }
            }
            ...
            "Faces=" => {
                ...
                for j in (i + 1)..(i + 1 + count) {
                    let line = lines.get(j).ok_or_else(|| {
                        format!("SCO truncated at face {}", j - i - 1)
                    })?;
                    ...
                    sco.uvs.push([f[5].parse().map_err(|e| format!("uv0.u: {e}"))?,
                                  f[6].parse().map_err(|e| format!("uv0.v: {e}"))?]);
                    sco.uvs.push([f[7].parse().map_err(|e| format!("uv1.u: {e}"))?,
                                  f[8].parse().map_err(|e| format!("uv1.v: {e}"))?]);
                    sco.uvs.push([f[9].parse().map_err(|e| format!("uv2.u: {e}"))?,
...
fn write_scb(sco: &Sco) -> Result<Vec<u8>, String> {
    ...
    let positions: Vec<[f32; 3]> = if needs_subtract {
        sco.positions.iter().map(|p| [p[0] - cx, p[1] - cy, p[2] - cz]).collect()
    } else {
        sco.positions.clone()
    };
    let central_out = if needs_subtract { [0.0_f32, 0.0, 0.0] } else { sco.central };
    ...
    for v in &min { out.write_f32::<LE>(*v).unwrap(); }
    for v in &max { out.write_f32::<LE>(*v).unwrap(); }
...
fn convert_one(path: &Path) -> Result<PathBuf, String> {
...
pub fn run(path: &Path) -> Result<(), String> {
    let scb_path = convert_one(path)?;
    println!("Converted: {} -> {}", path.display(), scb_path.display());
    Ok(())
}
...
fn walk_sco(dir: &Path, out: &mut Vec<PathBuf>) -> Result<(), String> {
    ...
    for e in entries {
        let e = e.map_err(|err| format!("Failed to read entry in {}: {}", dir.display(), err))?;
        let p = e.path();
        if p.is_dir() {
            walk_sco(&p, out)?;
        } else if p.extension().and_then(|x| x.to_str()).map(|x| x.eq_ignore_ascii_case("sco")).unwrap_or(false) {
            out.push(p);
        }
    }
    Ok(())
...
pub fn run_dir(dir: &Path) -> Result<(), String> {
    // Accept a file path as a convenience (right-clicking a file inside a
    // folder passes the file via %1 in some Windows configs).
    ...
    if files.is_empty() {
        eprintln!("No .sco files found under {}", scan_root.display());
        return Ok(());
    }
    ...
    for p in &files {
        match convert_one(p) {
            Ok(scb) => {
                println!("[OK] {} -> {}", p.display(), scb.display());
                ok += 1;
            }
            Err(e) => {
