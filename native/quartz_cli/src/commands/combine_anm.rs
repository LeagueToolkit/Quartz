...
pub fn run(bin_path: &Path) -> Result<(), String> {
    ...
    for (f, original_link) in &files_to_process {
        match read_bin(f) {
            Ok(mut linked_bin) => {
                let anm_keys: Vec<u32> = linked_bin
                    .objects
                    .iter()
                    .filter(|(_, obj)| obj.class_hash == anm_type_hash)
                    .map(|(&k, _)| k)
                    .collect();

                if anm_keys.is_empty() {
                    continue;
                }
                ...
                eprintln!(
                    "  [MERGE] {} AnimationGraphData entries from {} (link: {})",
                    merged_here,
    ...
    if total_merged == 0 {
        eprintln!("No AnimationGraphData entries found in linked bins.");
        return Ok(());
    }
    ...
    eprintln!(
        "\n[OK] SUCCESS: Merged {} AnimationGraphData entries into {}",
        total_merged,
        main_bin_path.file_name().unwrap_or_default().to_string_lossy()
    );
