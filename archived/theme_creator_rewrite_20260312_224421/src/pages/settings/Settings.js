...
const ModernSettings = () => {
  ...
  const handleCustomThemeValueChange = (field, value) => {
    ...
    let nextValues;
    setCustomThemeValues(prev => {
      nextValues = { ...prev, [field]: normalizedValue };
      return nextValues;
    });
    ...
    if (livePreview) {
      ...
      livePreviewTimer.current = setTimeout(() => {
        try { applyThemeFromObject(nextValues || customThemeValuesRef.current); } catch { }
      }, 120);
    }
  };
  ...
  useEffect(() => {
    ...
    if (isThemeCreatorDirty && selectedSection === 'themeCreator') return;
  ...
  }, [isThemeCreatorDirty, selectedSection, settings.themeVariant]);

  const handleToggleLivePreview = (enabled) => {
    setLivePreview(enabled);
    try {
      if (enabled) {
        applyThemeFromObject(customThemeValuesRef.current);
      } else {
