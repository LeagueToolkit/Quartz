...
export default function BnkExtract() {
    ...
    const estimateSnapshotBytes = useCallback((leftTree, rightTree) => {
        // Estimate bytes per snapshot based on unique audio buffers across both panes.
    ...
    const trimHistoryByBudget = useCallback((entries) => {
        // Enforce entry-count and byte-budget limits; drop oldest first.
        let next = [...entries];
    ...
    }, [undoStack, treeData, rightTreeData, estimateSnapshotBytes]);
    ...
    }, [redoStack, treeData, rightTreeData, estimateSnapshotBytes, trimHistoryByBudget]);
