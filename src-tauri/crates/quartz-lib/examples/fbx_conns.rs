...
fn av(v: &AttributeValue) -> String {
    match v {
        ...
        AttributeValue::String(s) => s.split('\0').next().unwrap_or("").to_string(),
        _ => "·".into(),
    }
}
fn main() {
        ...
        loop {
            match p.next_event().unwrap() {
                Event::StartNode(n) => {
                    ...
                    let mut strs = vec![];
                    loop {
                        let x = match a.load_next(DirectLoader) {
                            Ok(Some(x)) => x,
                            _ => break,
                        };
                        if let AttributeValue::I64(i) = x {
                            ids.push(i);
                        }
                        if let AttributeValue::String(ref s) = x {
                            strs.push(s.clone());
                        }
                        vals.push(av(&x));
                    }
                    ...
                    if nm == "C" && ids.len() >= 2 {
                        conns.push((strs.first().cloned().unwrap_or_default(), ids[0], ids[1]));
                    }
                    stack.push(nm);
                }
                Event::EndNode => {
    ...
    for (ty, a, b) in &conns {
        ...
        let key = format!(
            "{} {} -> {}",
            ty,
            ka.split(':').next().unwrap(),
            kb.split(':').next().unwrap()
        );
        let c = shown_types.entry(key.clone()).or_insert(0);
        if *c < 2 {
            println!("{}  ({} {} -> {})", key, ty, ka, kb);
        }
        *c += 1;
    }
    println!("--- connection type summary ---");
    let mut v: Vec<_> = shown_types.into_iter().collect();
    v.sort();
    for (k, n) in v {
        println!("{} x{}", k, n);
    }
