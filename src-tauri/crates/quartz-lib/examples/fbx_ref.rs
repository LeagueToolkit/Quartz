...
fn av(v: &AttributeValue) -> String {
    match v {
        ...
        AttributeValue::I32(x) => format!("{}", x),
        AttributeValue::String(s) => format!("{:?}", s),
        AttributeValue::F64(x) => format!("{:.1}", x),
...
fn main() {
    ...
    let mut printed_na = 0;
    let mut printed_model = 0;
        ...
        loop {
            match p.next_event().unwrap() {
                Event::StartNode(n) => {
                    ...
                    let mut a = n.attributes();
                    let mut v = vec![];
                    while let Ok(Some(x)) = a.load_next(DirectLoader) {
                        v.push(av(&x));
                    }
                    // Print first NodeAttribute subtree and first bone Model subtree
                    if parent == "Objects" && nm == "NodeAttribute" && printed_na < 1 {
                        in_target = true;
                        indent = stack.len();
                        printed_na += 1;
                    }
                    if parent == "Objects"
                        && nm == "Model"
                        && v.get(1)
                            .map(|s| s.contains("LimbNode") || s.contains("Model"))
                            .unwrap_or(false)
                        && printed_model < 1
                        && v.iter()
                            .any(|s| s.contains("LimbNode") || s.contains("Skeleton"))
                    {
                        in_target = true;
                        indent = stack.len();
                        printed_model += 1;
                    }
                    if in_target && stack.len() >= indent {
                        println!(
                            "{}<{}> {}",
                            "  ".repeat(stack.len() - indent),
                            nm,
                            v.join(", ")
                        );
                    }
                ...
                Event::EndNode => {
                    if in_target && stack.len() <= indent + 1 {
                        in_target = false;
                    }
                    stack.pop();
                }
