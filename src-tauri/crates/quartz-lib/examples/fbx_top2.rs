...
fn main() {
    ...
    let mut depth=0i32; let mut objkids: Vec<String>=vec![]; let mut inobj=0i32;
    if let AnyParser::V7400(mut p) = AnyParser::from_seekable_reader(reader).unwrap() {
        loop { match p.next_event().unwrap() {
            Event::StartNode(n)=>{
                ...
                if depth==0 { println!("TOP: {}", nm); }
                if nm=="Objects" { inobj=depth+1; }
                if depth==inobj && inobj>0 && objkids.len()<12 && !objkids.contains(&nm) { objkids.push(nm.clone()); }
                depth+=1;
            }
