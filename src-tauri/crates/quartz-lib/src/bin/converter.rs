...
//! printer/parser via `ritoshark_bridge`; JSON conversion uses the local `bin_json`
//! mapping (`rs_bin::BinValue` is not serde-derived).

use crate::bin::bin_json;
use crate::bin::ritoshark_bridge::{text_to_tree, tree_to_text_cached};
...
pub fn bin_to_text(tree: &Bin) -> Result<String> {
    tree_to_text_cached(tree).map_err(|e| bin_error(format!("Failed to convert to text: {}", e)))
}

/// Convert Python-like text format to Bin
pub fn text_to_bin(text: &str) -> Result<Bin> {
    text_to_tree(text).map_err(|e| bin_error(format!("Failed to parse text: {}", e)))
}
