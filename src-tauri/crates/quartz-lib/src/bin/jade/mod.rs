...
pub fn convert_bin_to_text(data: &[u8]) -> Result<String, String> {
    ...
    let mut bin = reader::read(data).map_err(|e| format!("Jade reader error: {}", e.0))?;
...
pub fn convert_text_to_bin(text: &str) -> Result<Vec<u8>, String> {
    let bin = text_reader::read(text).map_err(|errors| text_reader::format_errors(&errors))?;

    writer::write(&bin).map_err(|e| format!("Jade writer error: {}", e.0))
}
