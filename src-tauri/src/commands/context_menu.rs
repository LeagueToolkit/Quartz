...
mod imp {
    ...
    const fn v(key: &'static str, label: &'static str, verb: &'static str) -> Verb {
        Verb { key, label, verb, separator: false }
    }
    const fn vs(key: &'static str, label: &'static str, verb: &'static str) -> Verb {
        Verb { key, label, verb, separator: true }
    }
    ...
    // ── Folder menu ─────────────────────────────────────────────────────────
    const DIR: &[Verb] = &[
        v("00ritobindir2py", "ritobin: Convert all BIN to PY", "ritobindir2py"),
        v("01ritobindir2bin", "ritobin: Convert all PY to BIN", "ritobindir2bin"),
    ...
    const MENUS: &[Menu] = &[
        Menu { root: r"SystemFileAssociations\.bin\shell\Quartz", verbs: BIN, arg: "%1" },
        Menu { root: r"SystemFileAssociations\.py\shell\Quartz",  verbs: PY,  arg: "%1" },
        Menu { root: r"SystemFileAssociations\.tex\shell\Quartz", verbs: TEX, arg: "%1" },
        Menu { root: r"SystemFileAssociations\.dds\shell\Quartz", verbs: DDS, arg: "%1" },
        Menu { root: r"SystemFileAssociations\.png\shell\Quartz", verbs: PNG, arg: "%1" },
        Menu { root: r"Directory\shell\Quartz",                   verbs: DIR, arg: "%V" },
    ];
    ...
    pub fn enable() -> Result<(), String> {
        ...
        for menu in MENUS {
            ...
            root_key.set_value("MUIVerb", &"Quartz").map_err(|e| e.to_string())?;
            root_key.set_value("Icon", &exe).map_err(|e| e.to_string())?;
            // Empty SubCommands makes Explorer read the child `shell\*` verbs.
            root_key.set_value("SubCommands", &"").map_err(|e| e.to_string())?;
            ...
            for verb in menu.verbs {
                ...
                vkey.set_value("MUIVerb", &verb.label).map_err(|e| e.to_string())?;
                vkey.set_value("Icon", &exe).map_err(|e| e.to_string())?;
                if verb.separator {
                    // 0x20 = SECColumn / start a new group with a separator above.
                    vkey.set_value("CommandFlags", &0x20u32).map_err(|e| e.to_string())?;
                }
