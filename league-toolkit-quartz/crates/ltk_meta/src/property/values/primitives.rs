...
// Matrix44 uses raw [f32; 16] to preserve binary layout exactly.
...
pub struct Matrix44<M = NoMeta> {
    pub value: [f32; 16],
    pub meta: M,
}

impl<M: Default> Matrix44<M> {
    #[inline(always)]
    #[must_use]
...
impl<M> PropertyExt for Matrix44<M> {
    fn size_no_header(&self) -> usize {
        core::mem::size_of::<[f32; 16]>()
    }
}

impl<M> PropertyValueExt for Matrix44<M> {
    const KIND: Kind = Kind::Matrix44;
}

impl<M: Default> ReadProperty for Matrix44<M> {
    fn from_reader<R: std::io::Read + ?Sized>(
        reader: &mut R,
        _legacy: bool,
    ) -> Result<Self, crate::Error> {
...
impl<M> WriteProperty for Matrix44<M> {
    fn to_writer<W: std::io::Write + std::io::Seek + ?Sized>(
        &self,
        writer: &mut W,
        _legacy: bool,
...
impl<M> AsRef<[f32; 16]> for Matrix44<M> {
    fn as_ref(&self) -> &[f32; 16] {
        &self.value
    }
}

impl<M> std::ops::Deref for Matrix44<M> {
    type Target = [f32; 16];
